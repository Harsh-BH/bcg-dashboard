...
const triggerCls = "flex items-center gap-1 text-xs text-muted-foreground bg-transparent border-0 cursor-pointer hover:text-foreground py-1 px-2 rounded-md hover:bg-muted transition-colors";
  ...
  return (
    <div className="overflow-x-auto rounded-xl border border-border">
      <table className="w-full text-xs">
        <thead className="bg-slate-800 text-white dark:bg-slate-700">
          <tr>
            {keys.map((k, i) => (
    ...
    <div className="space-y-6">
        ...
        <Card className="rounded-2xl shadow-sm hover:shadow-md transition-shadow duration-200 border-border">
          ...
          <CardContent className="space-y-4">
            ...
            {span.service_line_span.length > 0 && (
              <>
                <p className="text-sm font-semibold text-foreground mt-4">Span (IC ÷ TL) by service line</p>
      ...
      <Card className="rounded-2xl shadow-sm hover:shadow-md transition-shadow duration-200 border-border">
        <CardHeader className="pb-2">
          ...
          {span.cluster_status && (
            <p className="text-xs text-muted-foreground">{span.cluster_status}</p>
          )}
        </CardHeader>
        <CardContent className="space-y-4">
          {span.cluster_summary.length > 0 && (
            <>
              <p className="text-sm font-semibold text-foreground">Span counts by cluster</p>
      ...
      <Collapsible>
        <CollapsibleTrigger className={`${triggerCls} text-muted-foreground`}>
          TL designation phrases (A1/A2 mandatory TL) <ChevronDown size={12} />
        </CollapsibleTrigger>
        <CollapsibleContent>
          <Card className="mt-2 rounded-xl border-border">
